function getDimensionFinalMultiplier(tier) {
  ...
  multiplier = maybeDilate(multiplier);

  if (player.dilation.upgrades.includes(6)) multiplier = multiplier.times(player.dilation.dilatedTime.pow(308))
  return multiplier;
}

function maybeDilate(x) {
  if (player.dilation.active) {
    return Decimal.pow(10, Math.pow(Math.abs(x.log10()), player.dilation.upgrades.includes(9) ? 0.8 : 0.75)).pow(Math.sign(x.log10()))
  } else {
    return x;
  }
}
...
function getDimensionProductionPerSecond(tier) {
    ...
    if (player.dilation.active) {
        let tick = maybeDilate(new Decimal(player.tickspeed));
        ret = Decimal.floor(player[TIER_NAMES[tier] + 'Amount']).times(getDimensionFinalMultiplier(tier)).dividedBy(tick.dividedBy(1000))
    }
    return ret;
}
